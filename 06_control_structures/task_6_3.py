...
for intf, vlan in trunk.items():
    ...
    for command in trunk_template:
        if command.endswith('allowed vlan'):
            if vl_string.startswith('add'):
                vl_string = vl_string.replace('add,', 'add ')
            elif vl_string.startswith('del'):
                vl_string = vl_string.replace('del,', 'del ')
            else:
                vl_string = vl_string.replace('only,', '')
            print(' {} {}'.format(command, vl_string))
        else:
            print(' {}'.format(command))
